      ...
      {openDropdown === "lang" && (
        <div className="absolute top-12 -left-12 w-32 text-white rounded-lg shadow-lg">
          ...
          <div className="py-2 px-3 bg-[#16182B] w-[117px] rounded-xl">
            <button onClick={() => changeLanguage("en")} className="block w-full text-center border-gray-600 border-b-2 px-4 py-2">English</button>
            <button onClick={() => changeLanguage("ru")} className="block w-full text-center border-gray-600 border-b-2 px-4 py-2">Русский</button>
            <button onClick={() => changeLanguage("uz")} className="block w-full text-center px-4 py-2">Uzbek</button>
          </div>
        </div>
      )}
    </div>
